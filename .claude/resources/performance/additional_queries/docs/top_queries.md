...
Reads `sys.dm_exec_query_stats`, which is plan-cache-scoped — like the missing/unused index DMVs, this resets on service restart and only reflects queries whose plan is still cached. Not a substitute for Query Store history on SQL Server 2016+, which retains data across restarts and plan-cache eviction (see `.claude/resources/performance/README.md` Step 0 for enabling it).
