...
Answers "is there corruption, and when did we last check" — before this script, `.claude/resources/maintenance/` had zero live corruption-related reads.
...
`master.dbo.CommandLog` only exists once Ola Hallengren's maintenance scripts are installed (see `.claude/resources/maintenance/README.md`). A database that has never run CHECKDB through that path reports `LastCheckDBEndTime = NULL` and `severity = CRITICAL` — not a false positive, a real gap.
