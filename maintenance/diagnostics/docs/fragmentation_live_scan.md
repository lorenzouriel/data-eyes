...
Live `sys.dm_db_index_physical_stats` scan (actual current fragmentation), distinct from `performance/additional_queries/unused_indexes.sql` (usage-stats based). Feeds the Index & Buffer tab and Step 2 of the performance methodology.
...
`< 5%` → `OK`; `5–30%` → `WARNING` (`REORGANIZE`); `> 30%` → `CRITICAL` (`REBUILD`). Matches the fragmentation bands already documented in `performance/README.md` Step 2 and the workbook's Index_Maintenance tab.
